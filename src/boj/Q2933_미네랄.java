...
public class Q2933_미네랄 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        R = Integer.parseInt(st.nextToken()); // 동굴 높이
        C = Integer.parseInt(st.nextToken()); // 동굴 넓이
    ...
    public static void throwing(char[][] map, int height, boolean left) {
        ...
        // 왼쪽(창영이가)에서 던지면 c=0, delta=1
        // 오른쪽(상근이가)에서 던지면 c=1, delta=-1
        int c       = (left) ? 0    : C-1;
        int delta    = (left) ? 1   : -1 ;
    ...
    public static void fall(char[][] map) {
        ...
        for(int c = 0; c < C; c++) {
            // 떨어져야 하는 미네랄이 처음 발견되면 DFS를 통해 연결된 클러스터 체크
            if(map[1][c]=='x' && !ground[1][c]) dfs(map, ground, 1, c);
        }

        // ground가 true이면 미네랄은 땅에 붙어있는 미네랄
        // ground가 false이고 map[r][c]이 'x'이라면 떨어져야 하는 미네랄


        // 공중에 떠있는 클러스터가 떨어져야할 높이 계산
        int minHeight = R;
        System.out.println("첫번째 minHeight="+minHeight);
        for(int c = 0; c < C; c++) {
            int height = 0;
            for(int r = 1; r <= R; r++) {
        ...
        System.out.println("두번째 minHeight="+minHeight);
